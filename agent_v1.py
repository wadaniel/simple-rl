...
def value_function(state):
    ...
    with tf.compat.v1.variable_scope("value_network"):
        ...
        init_xavier = tf.compat.v1.keras.initializers.glorot_normal()
        ...
        V = tf.compat.v1.layers.dense(hidden2, n_outputs, None, init_xavier)
    return V
...
training_op_actor = tf.compat.v1.train.AdamOptimizer(lr_actor, name='actor_optimizer').minimize(loss_actor)
...
training_op_critic = tf.compat.v1.train.AdamOptimizer(lr_critic, name='critic_optimizer').minimize(loss_critic)
################################################################
#Training loop
